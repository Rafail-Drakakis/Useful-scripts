...
# Install LaTeX and fonts (redundant if texlive-full is installed, but harmless)
...
# ===== LaTeXMK CONFIG: clean minted caches =====
LATEXMKRC="$HOME/.latexmkrc"
if [ -f "$LATEXMKRC" ]; then
  cp -n "$LATEXMKRC" "$LATEXMKRC.bak.$(date +%Y%m%d%H%M%S)"
fi
cat > "$LATEXMKRC" <<'EOF'
...
EOF
echo "Configured ~/.latexmkrc for minted cache cleanup."
